...
class AntsConfig(Config):
    ...
    #NUM_CLASSES = 1 + 2  # background + 1 invasive-ant-types (incl. resembling ants) + 1 Non-Target-Ants
    NUM_CLASSES = 1 + 3  # background + 1 invasive-ant-types (incl. resembling ants) + all other invasive ants + 1 Non-Target-Ants
...
class AntDetector(utils.Dataset):
        ...
        Dataset is created with the 3 classes defined as 1 background, 1 target and 1 non-target.
        All species that do not fall in the defined 1 classes will be assumed as non-target.
       ...
       generate masks for the annotated ants in the images files and allocate corresponding class to it.
        ...
        def load_dataset(self, dataset, is_train=True):
            ...
            self.add_class("dataset", 1, "Wasmannia_auropunctata")
        ...
        def load_mask(self, image_id):
            ...
            for i in range(len(boxes)):
                ...
                if(box[4] == 'Wasmannia_auropunctata'):
                    masks[row_s:row_e, col_s:col_e, i] = 1
                    class_ids.append(self.class_names.index('Wasmannia_auropunctata'))
                elif(box[4] == 'Lepisiota_frauenfeldi' or box[4] == 'Linepithema_humile' or box[4] == 'Solenopsis_geminata' or box[4] == 'Pheidole_megacephala' or box[4] =='Solenopsis_invicta' or box[4] =='Anoplolepis_gracilipes'):
